                ...
                <div class="row">
                	<div class='col-md-4' id="faq-menu-category">
                		@if($faq_category->count() > 0) 
	                		<h4 style="margin-top: 0 !important;">FAQ CATEGORY</h4>
	                		<div class="row-md-12 list-group">
	                			@foreach($faq_category as $faq_cat)
	                				@if($faq_cat->id == $faq_category_selected) 
	                					<a href="{{ route('support.with.faq_categories', ['faq_category_id' =>  $faq_cat->id])  }}" class="list-group-item active">{{ $faq_cat->name }}</a>
	                				@else
	                					<a href="{{ route('support.with.faq_categories', ['faq_category_id' =>  $faq_cat->id])  }}" class="list-group-item">{{ $faq_cat->name }}</a>
	                				@endif
	                			@endforeach
	                		</div>
                		@endif
                	...
                	<div class="col-md-8">
                		@if($faq_category_answer->count() > 0)
	                		<div class="bar-color">
